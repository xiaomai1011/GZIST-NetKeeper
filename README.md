...
1. **[下载最新版 ZIP](../../releases/latest)**（或克隆本仓库），解压到一个**固定位置**（建议 `D:\` 下，不要放桌面随手删）
2. 双击 `重新配置账号.bat`，输入你的学号和校园网密码（只填这一次，保存在本机）
3. 双击 `校园网登录.bat` 测试能否登录成功
4. 双击 `安装开机自启.bat` 部署开机自启 + 后台保活，完成

> 想验证效果：双击 `注销测试.bat` 主动把自己踢下线，然后什么都不做，等十几秒看是否自动恢复。
...
| `诊断检测.bat` | 一键检查网络、配置、守护进程状态（**出问题先跑这个**） |
...
- **不确定守护有没有在跑**
  运行 `诊断检测.bat`；也可以打开 `campusnet_log.txt`，看最新一条是否有守护心跳记录。
- **想换检测间隔**
